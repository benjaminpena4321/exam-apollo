<template>
    <div id="container"> 
        <svg preserveAspectRatio="xMidYMin slice" height="100" width="100" viewBox="0 0 728 400">
            <!--Background-->  
            ...
            <!--Text & link to path-->  
            <text  font-size="20" fill="#1d1f20">
                <textPath id="text" class="inside" xlink:href="#s"></textPath>
            </text>  
        </svg> 
    </div>
        ...
        const arr = [];
            $.ajax({    //create an ajax request to display.php
                    ...
                    success: function(response){                    
                        ...
                        // console.log(obj[0]['breakdowns'][0]['values'])

                    var countRandom = obj.length;
                    
                    // loop randoms
                    for(var i=0; i < countRandom; i++){
                        //    console.log(obj[i])

                        // nested loop breakdowns
                        for(var x=0; x < obj[i]['breakdowns'].length;x++ ){

                            // store data to array
                            arr.push(obj[i]['breakdowns'][x]['values'])
